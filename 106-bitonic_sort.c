...
/**
 * bitonic_merge - Sorts a bitonic sequence within an array of integers.
		...
		char flow)
{
	...
	if (seq > 1)
	{
		for (e = start; e < start + jump; e++)
		{
			if ((flow == UP && array[e] > array[e + jump]) ||
			    (flow == DOWN && array[e] < array[e + jump]))
				swap_ints(array + e, array + e + jump);
		}
